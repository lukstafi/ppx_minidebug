...
let%expect_test "%debug_show PrintBox to stdout track for-loop num children exceeded" =
  ...
  [%expect
    {|
    BEGIN DEBUG SESSION
    "test/test_expect_test.ml":264:26:
    └─"test/test_expect_test.ml":265:8: <for loop>
      ├─i = 0
      ├─"test/test_expect_test.ml":265:12:
      │ └─"test/test_expect_test.ml":266:14:
      │   └─_baz = 0
      ├─i = 1
      ├─"test/test_expect_test.ml":265:12:
      │ └─"test/test_expect_test.ml":266:14:
...
let%expect_test "%debug_show PrintBox to stdout track for-loop" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":306:26:
      ├─"test/test_expect_test.ml":307:8: <for loop>
      │ ├─i = 0
      │ ├─"test/test_expect_test.ml":307:12:
      │ │ └─"test/test_expect_test.ml":308:14:
      │ │   └─_baz = 0
      │ ├─i = 1
      │ ├─"test/test_expect_test.ml":307:12:
      │ │ └─"test/test_expect_test.ml":308:14:
...
let%expect_test "%debug_show PrintBox to stdout track while-loop" =
  ...
  [%expect {|
    BEGIN DEBUG SESSION
    "test/test_expect_test.ml":354:26:
    ├─"test/test_expect_test.ml":356:8: <while loop>
    │ ├─"test/test_expect_test.ml":357:10:
    │ │ └─"test/test_expect_test.ml":357:14:
    │ │   └─_baz = 0
    │ ├─"test/test_expect_test.ml":357:10:
    │ │ └─"test/test_expect_test.ml":357:14:
    │ │   └─_baz = 2
...
let%expect_test "%debug_show PrintBox to stdout num children exceeded nested" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":369:40-375:69: loop_exceeded
      ├─x = 3
      ├─"test/test_expect_test.ml":374:15:
      │ └─z = 1
      └─"test/test_expect_test.ml":369:40-375:69: loop_exceeded
        ├─x = 2
        ├─"test/test_expect_test.ml":374:15:
...
let%expect_test "%debug_show PrintBox to stdout highlight" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      ┌────────────────────────────────────────────────────────┐
      │"test/test_expect_test.ml":425:41-427:58: loop_highlight│
      ├────────────────────────────────────────────────────────┘
      ├─x = 7
      ├─┬──────────────────────────────────┐
      │ │"test/test_expect_test.ml":426:8: │
      │ ├──────────────────────────────────┘
      │ └─┬─────┐
      │   │z = 3│
      │   └─────┘
...
let%expect_test "%debug_show PrintBox tracking" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":493:37-495:46: track_branches
      ├─x = 7
      ├─"test/test_expect_test.ml":495:9: <if -- else branch>
      │ └─"test/test_expect_test.ml":495:31: <match -- branch 1>
      └─track_branches = 4
      4
      "test/test_expect_test.ml":493:37-495:46: track_branches
      ├─x = 3
      ├─"test/test_expect_test.ml":494:18: <if -- then branch>
      │ └─"test/test_expect_test.ml":494:49: <match -- branch 2>
      └─track_branches = -3
      -3
    |}]
...
let%expect_test "%debug_show PrintBox tracking <function>" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":526:6: <function -- branch 3>
      4
      "test/test_expect_test.ml":528:6: <function -- branch 5>
      -3
    |}]
...
let%expect_test "%debug_show PrintBox tracking with debug_notrace" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":547:37-561:16: track_branches
      ├─x = 8
      ├─"test/test_expect_test.ml":556:6: <if -- else branch>
      │ └─"test/test_expect_test.ml":559:8: <match -- branch 2>
      │   └─"test/test_expect_test.ml":560:14:
      │     ├─"test/test_expect_test.ml":560:44: <if -- then branch>
      │     └─result = 8
      └─track_branches = 8
      8
      "test/test_expect_test.ml":547:37-561:16: track_branches
      ├─x = 3
      ├─"test/test_expect_test.ml":549:6: <if -- then branch>
      │ └─"test/test_expect_test.ml":553:14:
      │   └─result = 3
      └─track_branches = 3
      3
    |}]
...
let%expect_test "nested extension points are no-ops" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":592:37-606:16: track_branches
      ├─x = 8
      ├─"test/test_expect_test.ml":601:6: <if -- else branch>
      │ └─"test/test_expect_test.ml":604:8: <match -- branch 2>
      │   └─"test/test_expect_test.ml":605:23:
      │     ├─"test/test_expect_test.ml":605:53: <if -- then branch>
      │     └─result = 8
      └─track_branches = 8
      8
      "test/test_expect_test.ml":592:37-606:16: track_branches
      ├─x = 3
      ├─"test/test_expect_test.ml":594:6: <if -- then branch>
      │ └─"test/test_expect_test.ml":598:25:
      │   └─result = 3
      └─track_branches = 3
      3
    |}]
...
let%expect_test "%track_show PrintBox to stdout no return type anonymous fun" =
  ...
  [%expect
    {|
      "test/test_expect_test.ml":648:32-649:70: anonymous
      ├─x = 3
      ├─"test/test_expect_test.ml":649:50-649:70: __fun
      │ └─i = 0
      ├─"test/test_expect_test.ml":649:50-649:70: __fun
      │ └─i = 1
      ├─"test/test_expect_test.ml":649:50-649:70: __fun
...
let%expect_test "%track_show PrintBox to stdout anonymous fun, num children exceeded" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":672:40-678:69: loop_exceeded
      ├─x = 3
      └─"test/test_expect_test.ml":676:9-678:69: __fun
        ├─i = 0
        ├─"test/test_expect_test.ml":677:15:
        │ └─z = 1
        └─"test/test_expect_test.ml":678:33: <if -- else branch>
          └─"test/test_expect_test.ml":672:40-678:69: loop_exceeded
            ├─x = 2
            └─"test/test_expect_test.ml":676:9-678:69: __fun
              ├─i = 0
                  ...
                  └─"test/test_expect_test.ml":676:9-678:69: __fun
                    ...
                    └─"test/test_expect_test.ml":678:33: <if -- else branch>
                      └─"test/test_expect_test.ml":672:40-678:69: loop_exceeded
                        ...
                        │ ├─"test/test_expect_test.ml":677:15:
                        │ │ └─z = 0
                        │ └─"test/test_expect_test.ml":678:26: <if -- then branch>
                        ├─"test/test_expect_test.ml":676:9-678:69: __fun
                        │ ├─i = 1
                        │ ├─"test/test_expect_test.ml":677:15:
                        │ │ └─z = 1
...
let%expect_test "%track_show PrintBox to stdout function with abstract type" =
  ...
  [%expect
    {|
      BEGIN DEBUG SESSION
      "test/test_expect_test.ml":772:26-773:47: foo
      ├─c = 1
      └─foo = 2
      2
    |}]
