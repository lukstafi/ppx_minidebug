...
let foo (x : int) =
  ((Debug_runtime.open_log_preamble_full ~fname:"test_debug_show.ml"
      ~start_lnum:3 ~start_colnum:19 ~end_lnum:5 ~end_colnum:15
      ~message:"foo";
...
let bar (x : t) =
  ((Debug_runtime.open_log_preamble_full ~fname:"test_debug_show.ml"
      ~start_lnum:10 ~start_colnum:19 ~end_lnum:10 ~end_colnum:73
      ~message:"bar";
...
let baz (x : t) =
  ((Debug_runtime.open_log_preamble_full ~fname:"test_debug_show.ml"
      ~start_lnum:13 ~start_colnum:19 ~end_lnum:14 ~end_colnum:67
      ~message:"baz";
    Debug_runtime.log_value_show ~descr:"x" ~v:(([%show : t]) x));
   (let baz__res =
...
let rec loop (depth : int) (x : t) =
  (((Debug_runtime.open_log_preamble_full ~fname:"test_debug_show.ml"
       ~start_lnum:17 ~start_colnum:24 ~end_lnum:23 ~end_colnum:9
       ~message:"loop";
        ...
        then
          ...
          (let y : int =
             Debug_runtime.open_log_preamble_brief
               ~fname:"test_debug_show.ml" ~pos_lnum:21 ~pos_colnum:8
               ~message:" ";
             (let y__res =
                (loop (depth + 1)
