...
let create () = { array = [||]; length = 0 }
let length t = t.length

let get t i =
  if i < 0 || i >= t.length then invalid_arg "Dynarray.get: index out of bounds"
  else t.array.(i)

let set t i v =
  if i < 0 || i >= t.length then invalid_arg "Dynarray.set: index out of bounds"
  else t.array.(i) <- v
...
let to_seq t =
  let rec aux i () =
    if i >= t.length then Seq.Nil else Seq.Cons (t.array.(i), aux (i + 1))
  in
  aux 0
