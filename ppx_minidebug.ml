...
let debug_fun ~sexp callback bind descr_loc t_lident_loc_opt1 exp =
...
let debug_binding ~sexp callback vb =
  ...
  | Pexp_fun _, _ -> 
    {vb with pvb_expr = debug_fun ~sexp callback vb.pvb_pat descr_loc t_lident_loc_opt vb.pvb_expr}
  | _, Some t_lident_loc ->
    ...
    method! expression e =
      ...
      | { pexp_desc=Pexp_let (rec_flag, bindings, body); pexp_loc=_; _ } ->
        let bindings = List.map (fun vb ->
              try debug_binding ~sexp callback vb
...
let debug_this_expander ~sexp ~ctxt:_ payload =
  ...
  | { pexp_desc = Pexp_let (recflag, bindings, body); _ } ->
    (* This is the [let%debug_this ... in] use-case: do not debug the whole body. *)
     let bindings = List.map (debug_binding ~sexp callback) bindings in
     {payload with pexp_desc=Pexp_let (recflag, bindings, body)}
  | expr -> expr

let debug_expander ~sexp ~ctxt:_ payload = (traverse ~sexp)#expression payload

let str_expander ~sexp ~loc ~path:_ payload =
  match List.map (fun si -> (traverse ~sexp)#structure_item si) payload with
  | [item] -> item
  | items ->
