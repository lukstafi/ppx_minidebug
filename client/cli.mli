...
(** Open database by run name - looks up the run in the metadata DB and opens the
    ...
    @param meta_file_base
      The base filename (without "_meta.db" suffix) for the metadata database
...
(** Print root entries efficiently. Fast for large databases. When [with_values] is true,
    includes immediate children values. *)
...
val search_tree :
  ...
  pattern:string ->
  int
(** Search with tree context - shows matching entries with their ancestor paths. Uses
    efficient ancestor propagation like the TUI. Returns number of matching scopes found.
...
val search_subtree :
  ...
  pattern:string ->
  int
(** Search and show only matching subtrees, pruning non-matching branches. Returns number
    of actual matches (not including propagated ancestors).

    This builds a minimal tree containing only paths to matches, unlike [search_tree]
    which shows full context paths.
...
val search_intersection :
  ...
  int
(** Search for scopes matching ALL patterns (intersection/AND logic). Returns number of
    scopes in the intersection.

    For each pattern, runs a separate search with ancestor propagation. Then computes the
    intersection of matching scope IDs across all searches. Shows full tree context
    (ancestors) for scopes in the intersection.
    ...
    Example: [search_intersection ~patterns:["(id 79)"; "(id 1802)"] client] finds scopes
    where both patterns appear in their subtree. *)
...
(** Show a specific scope subtree with full tree rendering.
    ...
    Arguments:
    - [max_depth]: INCREMENTAL depth from target scope (not absolute depth from root). For
      example, if target is at depth 5 and max_depth=3, shows up to depth 8.
    - [show_ancestors]: If true (default), includes ancestor path from root to scope.
    - [show_times]: Include elapsed times in output
    - [format]: Output format (Text or JSON)

    This command is useful after [search_intersection] to explore the full context of an
    LCA scope. *)
...
(** Search and show only unique entries at a specific depth on paths to matches.
    ...
    This provides a TUI-like summary view - shows only the depth-N ancestors of matching
    entries, giving a high-level overview without overwhelming detail.

    Example: [search_at_depth ~depth:4 ~pattern:"(id 79)" ~quiet_path:"env"] shows only
    the depth-4 scopes that are ancestors of matches, deduplicated.

    Use case: When [search_tree] returns too many results, use this to see a summary at a
    shallower depth, then drill down with [show_scope]. *)
...
(** Search DAG with a path pattern, then extract along a different path with
    ...
    For each match of search_path, extracts along extraction_path (which must share the
    first element with search_path). Prints each extracted subtree, skipping consecutive
